...
# =============================================================================
# MODIFIED: Constraint thresholds (FPR constraint removed)
# =============================================================================
MIN_ACCURACY = 0.90
...
def evaluate_individual(hparams):
    ...
    # Use validation accuracy from the best epoch restored by EarlyStopping
...
def compute_objectives_and_constraints(population):
    """
    MODIFIED: Evaluate each individual for 2 objectives (Accuracy, Size) and 2 constraints.
    The FPR is calculated and stored but not used as an objective or constraint.
    """
    results = []
    for ind in population:
        acc, size_mb, fpr = evaluate_individual(ind)
        
        # MODIFIED: Objectives are now [-Accuracy, ModelSize]
        f1 = -acc      # Minimize negative accuracy
        f2 = size_mb   # Minimize model size
        
        # The FPR is now just a tracked metric, not an objective
        tracked_fpr = fpr
        
        # MODIFIED: Constraint violations (g3 for FPR is removed)
        ...
        results.append({
            ...
            'fpr_metric': tracked_fpr, # Store FPR separately
            'CV': CV
        })
    return results
    ...
    for gen in range(max_gen):
        ...
        for ind in pop_data:
            rec = {
                ...
                'Accuracy': -ind['objs'][0],
                'Size_MB': ind['objs'][1],
                'FPR': ind['fpr_metric'], # MODIFIED: Get FPR from its new key
                'CV': ind['CV'],
                **ind['hparams']
            }
        ...
        if feasibles:
            ...
            # MODIFIED: No longer tracking "best_fpr" as it is not an objective
            print(f"  Best feasible metrics in current pop -> Acc: {best_acc:.4f}, Size: {best_size:.3f} MB")
        else:
...
output_dir = "/home/22EC1102/soumen/NIT-GPU_2/code/c_moo/2_stage/acc_size"
os.makedirs(output_dir, exist_ok=True)

pareto_solutions, all_gen_dfs = nsga2(POP_SIZE, MAX_GEN)

# --- Save all generation data to an Excel file ---
excel_path = os.path.join(output_dir, "all_generations_2_obj.xlsx")
...
if pareto_solutions:
    ...
    for sol in pareto_solutions:
        acc = -sol['objs'][0]
        size = sol['objs'][1]
        fpr = sol['fpr_metric'] # MODIFIED: Get FPR from its new key
        
        print(f"  Accuracy={acc:.4f}, Size={size:.3f} MB, FPR={fpr:.4f}, Hyperparams={sol['hparams']}")
    ...
    csv_path = os.path.join(output_dir, "final_pareto_2_obj.csv")
