...
# =============================================================================
# MODIFIED: Constraint thresholds (Size constraint removed, FPR constraint added)
# =============================================================================
MIN_ACCURACY = 0.90
...
def compute_objectives_and_constraints(population):
    """
    MODIFIED: Evaluate each individual for 2 objectives (Accuracy, FPR) and 2 constraints.
    The model size is calculated and stored but not used as an objective or constraint.
    """
    results = []
    for ind in population:
        acc, size_mb, fpr = evaluate_individual(ind)
        
        # MODIFIED: Objectives are now [-Accuracy, FPR]
        f1 = -acc  # Minimize negative accuracy
        f2 = fpr   # Minimize False Positive Rate
        
        # The size is now just a tracked metric, not an objective
        tracked_size = size_mb
        
        # MODIFIED: Constraint violations (g2 for size is removed, g3 for FPR is added)
        ...
        results.append({
            ...
            'size_metric': tracked_size, # Store size separately
            'CV': CV
        })
    return results
    ...
    for gen in range(max_gen):
        ...
        for ind in pop_data:
            rec = {
                'Generation': gen,
                'Accuracy': -ind['objs'][0],
                'Size_MB': ind['size_metric'], # MODIFIED: Get Size from its new key
                'FPR': ind['objs'][1],
                'CV': ind['CV'],
                **ind['hparams']
            }
        ...
        if feasibles:
            ...
            best_fpr = min(ind['objs'][1] for ind in feasibles) # MODIFIED: Report best FPR
            print(f"  Best feasible metrics in current pop -> Acc: {best_acc:.4f}, FPR: {best_fpr:.4f}")
        else:
...
output_dir = "/home/ec.gpu/Desktop/Soumen/nitd_0.4/nitd-0.4/kws/kws/c_moo/2_stage/acc_fpr"
os.makedirs(output_dir, exist_ok=True)

pareto_solutions, all_gen_dfs = nsga2(POP_SIZE, MAX_GEN)

# --- Save all generation data to an Excel file ---
excel_path = os.path.join(output_dir, "all_generations_acc_fpr.xlsx")
...
if pareto_solutions:
    print("\nFinal Pareto-Optimal Feasible Solutions (Accuracy vs. FPR):")
    pareto_records = []
    for sol in pareto_solutions:
        acc = -sol['objs'][0]
        fpr = sol['objs'][1]
        size = sol['size_metric'] # MODIFIED: Get Size from its new key
        
        print(f"  Accuracy={acc:.4f}, FPR={fpr:.4f}, Size={size:.3f} MB, Hyperparams={sol['hparams']}")
    ...
    csv_path = os.path.join(output_dir, "final_pareto_acc_fpr.csv")
