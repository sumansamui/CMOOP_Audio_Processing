...
# Latin Hypercube Sampling (LHS) initialization for hyperparameter space exploration
from pyDOE import lhs
import random

def lhs_initialization(pop_size, hparam_space):
    """
    Generate pop_size samples via LHS, then map back to discrete choices.
    hparam_space: dict of lists of possible values, e.g. {'filters':[16,32,64],...}
    """
    keys = list(hparam_space.keys())
    dims = len(keys)
    # generate LHS in [0,1]^dims
    unit = lhs(dims, samples=pop_size, criterion='maximin')
    population = []
    for row in unit:
        indiv = {}
        for i, k in enumerate(keys):
            choices = hparam_space[k]
            idx = min(int(row[i] * len(choices)), len(choices)-1)
            indiv[k] = choices[idx]
            print(f"Hyperparameter {k} sampled as {indiv[k]} from {choices}")
        population.append(indiv)
    print(f"Initialized population with {pop_size} individuals using LHS.")
    return population

###################################################

def initialize_population(pop_size):
    return lhs_initialization(pop_size, HPARAM_SPACE)
...
def nsga2(pop_size, max_gen, infill_percent):
    ...
    print(f"Initial population size: {len(population_hparams)}")
